            ...
            <form role="form" action="<?php echo site_url('Accounts') ?>/deposit_fund_form" method="post" class="validate" id="deposit_fund_form">
                    ...
                    <fieldset>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-md-12">
                            <?php if($ac['school_id']){?>
            <button type="button"   class="btn btn-primary btn-sm pull-right" id="deleteAccount"><i class="fa fa-remove"></i> Delete Account</button>
               <?php }?>
                            </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="from" class="form-control">
                                <?php $system = getSystemAccounts(true);
                                $dropdown = [];
...
<script>
    $(document).ready(function() {
        $('#deposit_fund_form').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= base_url() . 'Accounts/transferNow'; ?>', // Get the form's action attribute
    ...
    $("#deleteAccount").click(function () {
        if(confirm('Transactions will not be deleted , But account will be disabled, Are you sure?')){
        $.post('<?= base_url('Accounts/diableAccounts'); ?>', {
            account_id: '<?= $id; ?>'
        }, function(data) {
