...
//print_r($subject);
            ...
            <div class="">
                ...
                <div class="row">
                    <div class="col-md-1">
                        <input type="checkbox" name="make_copy" id="make_copy" value="1"> <label for="make_copy"> Make Copy</label>
                    </div>
                    <div class="col-md-11 " >
                       <span style="display: none" id="copy_days">
                        <?php

                        foreach ($getDaysnameList as $days_key => $days_value) {
                            if($days_value!='Monday' && $days_value!='Sunday'){
                                echo "<input name='copy[$days_value]' type='checkbox'> $days_value ";
                            }

                        }
                        ?>

                             <a class="btn btn-primary btn-sm make_copy_now">Make Copy</a>
</span>
                    </div>
...
$(".make_copy_now").click(function () {

    $.ajax({
        type: "POST",
        url: "CopyTimeTable",
        data: $("#form_Monday").serialize(), // Serialize form data
        success: function (data) {
      successMsg(data);
           $("#copy_days").hide()
            $("#make_copy").attr('checked',false);
        }
    });
});
