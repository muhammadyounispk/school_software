...
class Notification_model extends MY_Model {
    ...
    public function get($id = null) {
        ...
        if ($id != null) {
            ...
            $sql .= "where send_notification.school_id =" . getSchoolID();
        }
    ...
    public function getUnreadStudentNotification() {
        ...
        $sql = "select send_notification.* from send_notification  left JOIN read_notification on  read_notification.student_id=".$this->customlib->getStudentSessionUserID()." and read_notification.notification_id = send_notification.id WHERE send_notification.school_id='".getSchoolID()."' and send_notification.visible_student='yes' and read_notification.id IS NULL  group by send_notification.id order by send_notification.id desc";
        $query = $this->db->query($sql);
        return $query->result();
    }
...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.student_id=" . $this->db->escape($studentid) . " where send_notification.visible_student='Yes' AND send_notification.school_id=".getSchoolID()." order by send_notification.publish_date desc");
        return $query->result_array();
    }
...
IF (read_notification.id IS NULL,'unread','read') as notification_id
FROM send_notification
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where send_notification.visible_parent='Yes' and send_notification.school_id=".getSchoolID()." order by send_notification.publish_date desc");
        return $query->result_array();
    }

    public function countUnreadNotificationStudent($studentid = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("select * from (SELECT  IF (read_notification.id IS NULL,'unread','read') as notification_id FROM send_notification left JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.student_id=" . $this->db->escape($studentid) . " where send_notification.school_id=".getSchoolID()." and   send_notification.visible_student='Yes' and send_notification.publish_date <='" . $date . "') final where notification_id ='unread'");
    ...
    public function countUnreadNotificationParent($parentid = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("select * from (SELECT  IF (read_notification.id IS NULL,'unread','read') as notification_id FROM send_notification LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where  send_notification.school_id=".getSchoolID()." and  send_notification.school_id=".getSchoolID()." and  send_notification.visible_parent='Yes'  and send_notification.publish_date <='" . $date . "') final where notification_id ='unread'");
        return $query->num_rows();
    }
