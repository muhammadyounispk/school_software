...
class Onlineexam_model extends MY_model
{
    ...
    public function get($id = null, $publish = null)
    {
        ...
        $this->db->where('onlineexam.school_id', getSchoolID());
        $query = $this->db->get();
        if ($id != null) {
            return $query->row();
        } else {
    ...
    {
        $insert_data['school_id']=getSchoolID();
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
    ...
    public function getExamQuestions($id = null, $random_type = false)
    {
        ...
        $this->db->where('onlineexam_questions.school_id', getSchoolID());
        if ($random_type) {
    ...
    public function onlineexamReport($condition)
    {
        ...
        // $query = $this->db->get();
        // return $query->result();
        $query = "SELECT onlineexam.*,(select count(*) from onlineexam_students WHERE onlineexam_students.school_id=".getSchoolID()." and  onlineexam_students.onlineexam_id = onlineexam.id) as assign,(select count(*) from onlineexam_questions where onlineexam_questions.onlineexam_id=onlineexam.id) as questions FROM `onlineexam`  where " . $condition . " order by onlineexam.id asc";
