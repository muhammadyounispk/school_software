...
public $host;
    ...
    public function get($id = null) {
        ...
        $this->db->where('sch_settings.host', $this->host);
    ...
    public function ModuleRestriction($code=null) {
        $this->db->select('plan_features.name,plan_features.value,plan_features.is_yes,plan_features.code');
        $this->db->from('sch_settings');
        $this->db->join('plan_features', 'plan_features.plan_id = sch_settings.plan');
        $this->db->where('sch_settings.host', $this->host);
        if($code){
            $this->db->where("plan_features.code",$code);
            $query = $this->db->get();
            $result = $query->row();
    ...
    public function getActiveSessionByID($school_id)
    {
        $this->db->select('session_id')->from('sch_settings');
        $this->db->where('id', $school_id);
        $query = $this->db->get();
        return $query->row_array()['session_id'];
    }
    ...
    public function getSetting() {
        ...
        $this->db->select('sch_settings.stamp,,sch_settings.host,sch_settings.is_withdawal_locked,sch_settings.id,sch_settings.attendence_type,sch_settings.lang_id,sch_settings.is_rtl,sch_settings.fee_due_days,sch_settings.class_teacher,sch_settings.cron_secret_key,sch_settings.timezone,
        ...
        $data= $query->row();
        $chool_id= $data->id;
        $this->db->select("*")->from("print_headerfooter");
        $this->db->where("school_id",$chool_id);
        $this->db->where("print_type",'student_receipt');
       $recept= $this->db->get();
       $data->voucher_header=$recept->row()->header_image;   
       $data->voucher_footer=$recept->row()->footer_content;   
        return $data;
    }
    public function getSettingBySchoolID($school_id) {
    ...
    public function getAdminlogo() {
        ...
        $query = $this->db->select('admin_logo')->where("host", $this->host)->get('sch_settings');
        $logo = $query->row_array();
        echo $logo['admin_logo'];
    }

    public function getAdminsmalllogo() {
        $query = $this->db->select('admin_small_logo')->where("host", $this->host)->get('sch_settings');
        $logo = $query->row_array();
    ...
    public function get_appname() {
        ...
        $query = $this->db->select('name')->where("host", $this->host)->get('sch_settings');
    ...
    public function get_printheader() {
        return $this->db->select('*')->from('print_headerfooter')->where("school_id",getSchoolID())->get()->result_array();
    }
