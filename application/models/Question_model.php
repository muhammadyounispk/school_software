...
class Question_model extends MY_model
{
    ...
    public function get($id = null)
    {
        ...
        $this->db->where('questions.school_id', getSchoolID());
     ...
     public function getAllRecord() {
        ...
        $role_id = $userdata["role_id"];
           ...
           $this->datatables
                ...
                ->where('questions.school_id', getSchoolID())
                ->from('questions');
                if(isset($role_id) && ($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")){
                   $carray = array();
        $class_list=array();
        ...
        if(!empty($class_list)){
            ...
            $this->datatables->where_in('questions.school_id',getSchoolID());
    ...
    {
        return $this->db->select('*')->where("school_id",getSchoolID())->from('questions')->join('question_answers', 'question.id=question_answers.question_id')->get()->row_array();

    }
    public function get_option($id)
    {
        return $this->db->select('id,option')->where("school_id",getSchoolID())->from('question_options')->where('question_id', $id)->get()->result_array();
    }

    public function get_answer($id)
    {
        return $this->db->select('option_id as answer_id')->where("school_id",getSchoolID())->from('question_answers')->where('question_id', $id)->get()->row_array();
    }

    public function count(){
        $query = $this->db->select('count(*) as total')->where("school_id",getSchoolID())->get('questions')->row_array();
        return $query['total'];
    }
}
