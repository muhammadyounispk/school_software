...
class Stuattendence_model extends MY_Model {
    ...
    public function add($data) {
        ...
        if (isset($data['id'])) {
            ...
            $already=db::getCell("SELECT id from student_attendences 
            where date='".$data['date']."' and 
            student_session_id='".$data['student_session_id']."' and
            ...
            $this->log($message, $record_id, $action);
                ...
                $update_sql=db::prepUpdateQuery($data,"student_attendences","id",$already);
    ...
    public function searchAttendenceClassSection($class_id, $section_id, $date) {
        ...
        students.mobileno,
        student_sessions.attendence_dt,students.firstname,
        students.middlename,students.lastname,
        student_sessions.date,student_sessions.remark,
        student_sessions.biometric_attendence,students.roll_no,
        ...
        attendence_type.type as `att_type`,attendence_type.key_value as `key` from students ,
        (SELECT student_session.id,student_session.student_id ,IFNULL(student_attendences.date, 'xxx') as date,IFNULL(student_attendences.created_at, 'xxx') as attendence_dt,
    ...
    public function attendanceYearCount() {
        ...
        $query = $this->db->select("distinct year(date) as year")->where('school_id',getSchoolID())->get("student_attendences");
    ...
    public function checkholidatbydate($date) {
        ...
        $query = $this->db->select('count(*) as day ')->where($where)->where("school_id",getSchoolID())->get('student_attendences')->row_array();
        return $query['day'];
    }
