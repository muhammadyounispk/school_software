...
class Cms_media_model extends CI_Model {
    ...
    public function bulk_add($data = null) {
        foreach($data as $key=>$val){
            $data[$key]['school_id']=getSchoolID();
        }
    ...
    public function getSlug($slug = null) {
        $sql = "SELECT img_name FROM `front_cms_media_gallery` WHERE school_id=".getSchoolID()." and  img_name = '" . $slug . "' OR img_name LIKE '" . $slug . "-[0-9]*' ORDER BY LENGTH(img_name), img_name DESC LIMIT 1";
