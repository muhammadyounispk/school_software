...
class Bookissue_model extends MY_Model {
    ...
    public function dueforreturn($start_date, $end_date) {
        ...
        $sql = "SELECT count(*) as total FROM `book_issues` LEFT JOIN `books` ON `books`.`id` = `book_issues`.`book_id` left join libarary_members on libarary_members.id=book_issues.member_id left join staff on staff.id=libarary_members.member_id WHERE book_issues.school_id=".getSchoolID()." and  `book_issues`.`is_returned` = '0' " . $condition;
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    public function forreturn($start_date, $end_date) {
        ...
        $sql = "SELECT count(*) as total FROM `book_issues` LEFT JOIN `books` ON `books`.`id` = `book_issues`.`book_id` left join libarary_members on libarary_members.id=book_issues.member_id left join staff on staff.id=libarary_members.member_id WHERE book_issues.school_id=".getSchoolID()." and `book_issues`.`is_returned` = '1' " . $condition;
        $query = $this->db->query($sql);
        return $query->result_array();
    }
