...
class Content_model extends MY_Model {
    ...
    public function get($id = null) {
        ...
        $this->db->limit(10);
        $this->db->where('contents.school_id', getSchoolID());
        $query = $this->db->get();
        if ($id != null) {
    ...
    public function getContentByRole($id = null, $role = null) {
        ...
        $query = "SELECT contents.*,(select GROUP_CONCAT(role) FROM content_for WHERE   content_id=contents.id and contents.school_id=".getSchoolID().") as role,class_sections.id as `class_section_id`,classes.class,sections.section  FROM `content_for`  INNER JOIN contents on contents.id=content_for.content_id left JOIN class_sections on class_sections.id=contents.cls_sec_id left join classes on classes.id=class_sections.class_id LEFT JOIN sections on sections.id=class_sections.section_id" . $inner_sql . "  WHERE contents.school_id=".getSchoolID()."  GROUP by contents.id";
        ...
        if (isset($data['id'])) {
            ...
            if (isset($content_role) && !empty($content_role)) {
                ...
                for ($i = 0; $i < $total_rec; $i++) {
                    ...
                    $content_role[$i]['school_id'] = getSchoolID();
                }
