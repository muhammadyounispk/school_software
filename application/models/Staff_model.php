...
public $school_id;
    public function __construct()
    {
        ...
        $this->school_id=$this->school_model->get()['id'];
    }
    ...
    public function get($id = null)
    {
        ...
        $this->db->where('staff.school_id', getSchoolID());
    ...
    public function getTodayDayAttendance()
    {
        ...
        $this->db->where("(staff_attendance_type_id='1' OR staff_attendance_type_id='2' OR staff_attendance_type_id='4' and school_id=". getSchoolID().")");
    ...
    public function getAll_users($id = null, $is_active = null)
    {
        ...
        if ($id != null) {
            ...
            if ($is_active != null) {
                ...
                $this->db->where('staff.is_active', $is_active);
                $this->db->where('staff.school_id', $this->school_id);
            }
    ...
    public function adddoc($data)
    {
        ...
        if (isset($data['id'])) {
           ...
           $data['school_id']= $this->db->where('staff.school_id', $this->school_id);
    ...
    public function getStaffDesignation()
    {
        ...
        $query = $this->db->select('*')->where("is_active", "yes")->where("school_id", getSchoolID())->get("staff_designation");
        return $query->result_array();
    }

    public function getDepartment()
    {
        $query = $this->db->select('*')->where("is_active", "yes")->where("school_id", getSchoolID())->get('department');
        return $query->result_array();
    }
    ...
    public function getByEmail($email)
    {
        ...
        $this->db->where(" staff.email='$email' and staff.school_id='".$this->school_id."' ");
    ...
    {
        $this->db->select('roles.id')->from('roles');
        $this->db->where('roles.name', $name);
        $this->db->where('roles.school_id', getSchoolID());
        $query = $this->db->get();
        return $query->row_array()['id'];
    }


    public function getStaffbyrole($role_id)
    {
        $array=array("1"=>"Admin","2"=>"Teacher","3"=>"Accountant","4"=>"Librarian","6"=>"Receptionist","7"=>"Super Admin");

        $id=$this->getRoleIDbyRoleName($array[$role_id]);
        $this->db->select('staff.*,staff_designation.designation as designation,staff_roles.role_id, department.department_name as department,roles.name as user_type');
        $this->db->join("staff_designation", "staff_designation.id = staff.designation", "left");
        $this->db->join("department", "department.id = staff.department", "left");
        $this->db->join("staff_roles", "staff_roles.staff_id = staff.id", "left");
    ...
    {
        return $this->db->select("CONCAT_WS(' ',staff.name,staff.surname) as name,staff.employee_id")->from('staff')->where('staff.is_active', 1)->where('staff.school_id', getSchoolID())->get()->result_array();
