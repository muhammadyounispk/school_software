...
class Audit_model extends MY_Model {
    ...
    public function getAllRecord() {
        $this->datatables
                ...
                ->orderable('message, name, ip_address, action, platform, agent')->where('staff.school_id', getSchoolID())
                ->from('logs');
        return $this->datatables->generate('json');
    }

    public function count() {
        $query = $this->db->select('count(*) as total')->where('school_id', getSchoolID())->get('logs')->row_array();
        return $query['total'];
    }
