...
class Paymentsetting_model extends MY_Model {
    ...
    public function KuickPayPayments($data) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('month', $data['month']);
        $this->db->where('user_id', $data['user_id']);
        $q = $this->db->get('kuickpay_payments');

        if ($q->num_rows() > 0) {
            $this->db->where('id', $q->row()->id);
            $this->db->update('kuickpay_payments', $data);
            ...
            $this->db->insert('kuickpay_payments', $data);
    ...
    public function getKuickPayPayments($user_id,$month) {
        $this->db->select()->from('kuickpay_payments');
    ...
    public function DeleteTransactionKuickPay($tansaction_id) {
        $this->db->where('transaction_id', $tansaction_id);
        $this->db->delete('kuickpay_payments');
