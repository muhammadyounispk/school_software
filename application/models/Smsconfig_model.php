...
class Smsconfig_model extends MY_Model {
    ...
    public function getActiveSMS() {
        ...
        $query = $this->db->get();
       $logo= $this->db->select("admin_logo")->from('sch_settings')->where("id",$this->session->school_id)->get()->row()->admin_logo;
       $logo_url="https://".$_SERVER['HTTP_HOST']."/uploads/school_content/admin_logo/".$logo;
        $data= $query->row();
        if($data){
            $data->logo=$logo_url;
        }
