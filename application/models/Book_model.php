...
class Book_model extends MY_Model {
    ...
    public function get($id = null) {
        ...
        if ($id != null) {
          ...
          $this->db->where('books.school_id', getSchoolID());
        $query = $this->db->get();
        if ($id != null) {
    ...
    public function bookgetall() {
        ...
        $sql = "SELECT books.*,IFNULL(total_issue, '0') as `total_issue` FROM books LEFT JOIN (SELECT COUNT(*) as `total_issue`, book_id from book_issues  where is_returned= 0  GROUP by book_id) as `book_count` on books.id=book_count.book_id where 0=0 and books.school_id=".getSchoolID()."  ";
    ...
    public function listbook() {
        $this->db->select()->from('books');
        $this->db->limit(10);
          $this->db->order_by("school_id", getSchoolID());
    ...
    public function bookinventory($start_date, $end_date) {
        $condition = " and date_format(`books`.`postdate`,'%Y-%m-%d') between '" . $start_date . "' and '" . $end_date . "' AND books.school_id=".getSchoolID();
