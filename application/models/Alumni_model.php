...
class Alumni_model extends MY_Model {
    ...
    function getevents() {
        return $this->db->select('*')->from('alumni_events')->where('alumni_events.school_id', getSchoolID())->order_by('alumni_events.from_date', 'desc')->get()->result_array();
    }

    function get_eventbydate($date) {
        return $this->db->select('*')->from('alumni_events')->where('from_date', $date)->where('school_id', getSchoolID())->get()->result_array();
    }
    ...
    function alumniMail($class_id = null, $session = null, $section = null) {
        ...
        $this->db->where('alumni_students.school_id', getSchoolID());
        $query = $this->db->get();
