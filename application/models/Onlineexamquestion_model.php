...
class Onlineexamquestion_model extends CI_Model
{
    ...
    {
        $this->db->select('questions.*,subjects.id as `subject_id`,subjects.name as subject_name, IFNULL(onlineexam_questions.id,0) as `onlineexam_question_id`,IFNULL(onlineexam_questions.marks,1) as `onlineexam_question_marks`')->from('questions');
