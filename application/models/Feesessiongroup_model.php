...
class Feesessiongroup_model extends MY_Model {
    ...
    public function getFeesByGroup($id = null,$is_active=1) {
        ...
        $this->db->where('fee_groups.is_active', $is_active);
        if ($id != null) {
    ...
    public function getVoucherBySchoolID($school_id = null,$is_active=1) {
        $session_id=getSessionID($school_id);
        ...
        $this->db->where('fee_groups.is_system', 0);
        $this->db->where('fee_groups.is_active', $is_active);
        $this->db->where('fee_session_groups.school_id', $school_id);
        $query = $this->db->get();
    ...
    public function getFeesByGroupSingle($id = null) {
        $this->db->select('fee_session_groups.*,fee_groups.name as `group_name`');
        $this->db->from('fee_session_groups');
        $this->db->join('fee_groups', 'fee_groups.id = fee_session_groups.fee_groups_id');
        $this->db->where('fee_session_groups.session_id', $this->current_session);
    ...
    public function isPaid($fee_type_id,$fee_student_voucher_id) {
        $this->db->select('*');
        $this->db->from('student_fees_deposite');
        $this->db->where('student_fees_master_id', $fee_student_voucher_id);
        $this->db->where('fee_groups_feetype_id', $fee_type_id);
    ...
    public function studentFeeMaster($id) {
        $this->db->select('student_fees_master.*');
        $this->db->from('student_fees_master');
        $this->db->where('student_fees_master.id', $id);
         $query = $this->db->get();
        $data= $query->row();
        return unserialize($data->feetypes);
    }
    ...
    function group_exists($fee_groups_id) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $data = array('fee_groups_id' => $fee_groups_id, 'session_id' => $this->current_session,'school_id'=>getSchoolID());
