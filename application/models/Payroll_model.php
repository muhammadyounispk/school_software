...
class Payroll_model extends MY_Model
{
    ...
    public function checkPayslip($month, $year, $staff_id)
    {
        ...
        $query = $this->db->where(array('month' => $month, 'school_id'=>getSchoolID(), 'year' => $year, 'staff_id' => $staff_id))->get("staff_payslip");
    ...
    public function searchPaylist($name, $month, $year)
    {
        ...
        $query = $this->db->select('staff.*,staff_designation.designation as desg,department.department_name as department')->where(array('staff.name' => $name, 'staff.school_id'=>getSchoolID(), 'staff_payslip.month' => $month, 'staff_payslip.year' => $year))->join("staff_payslip", "staff.id = staff_payslip.staff_id")->join("staff_designation", "staff.designation = staff_designation.id")->join("department", "staff.department = department.id")->get("staff");
    ...
    public function count_attendance($month, $year, $staff_id, $attendance_type = 1)
    {
        ...
        $query      = $this->db->select('count(*) as att')->where(array('staff_id' => $staff_id,'school_id' => getSchoolID(),  'month(date)' => $month, 'year(date)' => $year, 'staff_attendance_type_id' => $attendance_type))->get("staff_attendance");
        return $query->result_array();
    }
    ...
    public function updatePaymentStatus($status, $id)
    {
        ...
        $this->db->where("id", $id)->where("school_id",getSchoolID())->update("staff_payslip", $data);
    }
    ...
    public function getpayrollReport($month, $year, $role)
    {
        ...
        $data['staff.school_id'] = getSchoolID();
    ...
    public function getbetweenpayrollReport($start_date, $end_date)
    {
        ...
        $condition = "date_format(staff_payslip.payment_date,'%Y-%m-%d') between '" . $start_date . "' and '" . $end_date . "'  and staff_payslip.school_id=".getSchoolID();
