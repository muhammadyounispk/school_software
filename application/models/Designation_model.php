...
class Designation_model extends MY_model {
    ...
    public function get($id = null) {
        ...
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->where("school_id", getSchoolID())->get("staff_designation");
            return $query->row_array();
        } else {
            $query = $this->db->where("is_active", "yes")->where("school_id", getSchoolID())->get("staff_designation");
    ...
    function check_designation_exists($name, $id) {
        ...
        if ($id != 0) {
            $data = array('id != ' => $id, 'designation' => $name,'school_id'=>getSchoolID());
