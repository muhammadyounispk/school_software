...
class Studentfee extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $feegroup = $this->feegroup_model->drowpdown();
    ...
    public function search()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            $this->class= $class       = $this->input->post('class_id');
          $this->section=  $section     = $this->input->post('section_id');
          $this->fee_month=  $fee_month     = $this->input->post('fee_month');
            $search      = $this->input->post('search');
            $search_text = $this->input->post('search_text');
            if (isset($search)) {
                if ($search == 'search_filter') {
                    $this->form_validation->set_rules('class_id', $this->lang->line('class'), 'trim|required|xss_clean');
                    $this->form_validation->set_rules('fee_month', $this->lang->line('fee_month'), 'trim|required|xss_clean');
                    if ($this->form_validation->run() == false) {
    ...
    public function reportbyname()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $student_due_fee              = $this->studentfeemaster_model->getStudentFees($student['student_session_id'], $this->current_voucher_id);
    ...
    public function addfee($id)
    {
        ...
        $data['student'] = $student;
        $fee_month=str_decode($_GET['fee_month']);
       
        $student_due_fee = $this->studentfeemaster_model->getStudentFees($student['student_session_id'],$fee_month);
        if($student_due_fee){
    ...
    public function addfeegroup()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($student_sesssion_array)) {
                ...
                $feetypes=array_column($fee[$value],"amount","feetype_id");
                $amount=array_sum($feetypes);
$insert_array = array(
    'student_session_id'   => $value,
    'fee_session_group_id' => $fee_session_groups,
    'feetypes' => serialize($feetypes),
    'amount' => $amount,
);
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($total_row as $total_row_key => $total_row_value) {
                ...
                $collected_array[] = array(
                    ...
                    'fee_session_group_id '          => $_POST['fee_month'],
                );
            ...
            if(in_array('whatsapp',$com)){
                $url='https://'.$_SERVER['HTTP_HOST'].'/PDF/download_vouchers?fee_month='.str_encode($_POST['fee_month']).'&student_id='.str_encode($student_session_id).'&path=1&whatsapp=1&fee=1';
                header('location:'.$url);
