...
class Paymentsettings extends Admin_Controller {
    ...
    function index() {
        ...
        $this->session->set_userdata('sub_menu', 'admin/WithDrawal');
    ...
    public function payu() {
        ...
        if($this->input->post('is_puitc')==1){
            ...
            $this->form_validation->set_rules('iban_no', $this->lang->line('iban_no'), 'trim|required|xss_clean');
        }else{
        ...
        $this->form_validation->set_rules('api_password', $this->lang->line('api_password'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run()) {
            $data = array(
                ...
                'is_puitc' => $this->input->post('is_puitc'),
            ...
            if($this->input->post('is_puitc')==1){
                $data = array(
                 'bank_name' => form_error('bank_name'),
                    ...
                    'iban_no' =>form_error('iban_no')
                );
            }else{
                $data = array(
                        'key' => form_error('key'),
                       'salt' => form_error('salt'),
                       'api_password' =>form_error('api_password')
                   );
