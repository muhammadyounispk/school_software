...
class Staff extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('role', $this->lang->line('role'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('cnic', $this->lang->line('cnic'), 'trim|required|xss_clean');
...
//==========================
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $arr=$this->input->post('salary');
            $totalAmount=0;
            foreach ($arr['amount'] as $amount) {
                $totalAmount += $amount;
            }
            ...
            $data1 = array(
                ...
                'linkedin'             => $linkedin,
                'instagram'            => $instagram,
                'meta'            => json_encode($this->input->post('salary')),
                'salary'            => $totalAmount
            );
           ...
           header("location:".$this->input->post("back_url"));
        }
    }
