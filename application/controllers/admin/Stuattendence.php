...
class Stuattendence extends Admin_Controller {
    ...
    function SaveAttendance()  {
        ...
        $holiday = $this->input->post('holiday');
            ...
            foreach ($session_ary as $key => $value) {
                $sql="SELECT*, 
                (SELECT class FROM classes where id=ss.class_id)  as class_name 
                from student_session ss inner join students s on s.id=ss.student_id where  ss.id=$value";
                $student=db::getRecord($sql);
                $typeid= $this->input->post('attendencetype' . $value) ;
                if($typeid){
                    $type=db::getCell("select type from attendence_type where id=$typeid");
                }
               
                $remarks= $this->input->post('remark' . $value);
                if($remarks){
                    $remarks="*REMARKS*: $remarks";
                }
                if(isset($notify[$value])){
                $notify_via=$notify[$value];
                if(in_array("sms",$notify_via)){
                    $text="Attendance  Dated: $date\nName: {$student['firstname']} {$student['lastname']}\n*Father Name: {$student['father_name']}\nClass: {$student['class_name']}\nStatus: {$type}\n{$remarks}";
                    $to=$student['mobileno'];
                     if($typeid!=1){
                    array_push($sms_arr,array("to"=>$to,"text"=>$text));
                     }
                }
                if(in_array("whatsapp",$notify_via)){
                    $text="*Attendance  Dated:* $date\n*Name:* {$student['firstname']} {$student['lastname']}\n*Father Name:* {$student['father_name']}\n*Class:* {$student['class_name']}\n*Status:* {$type}\n{$remarks}";
                    $to=$student['mobileno'];
                    if($typeid!=1){
                    array_push($whatsapp_arr,array("to"=>$to,"text"=>$text));
                    }

                }
                if(in_array("email",$notify_via)){
                    $text="Name: {$student['firstname']} {$student['lastname']}<br>Status: {$type}<br>{$remarks}";

                    $message_title="Attendance Alert Dated $date";
                    if($typeid!=1){
                        $this->mailer->send_mail($student['email'], $message_title, $text);
                        sleep(1);
                    }
            ...
            if($whatsapp_arr){
               $this->sendWhatsapptext($whatsapp_arr,true);
            }
            if($sms_arr){
                $this->sendSMS($sms_arr,true);
            }

           echo json_encode(array("code"=>200,"message"=>"Attendance Successfully updated"));
