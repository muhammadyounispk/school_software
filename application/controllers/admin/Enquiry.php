...
class Enquiry extends Admin_Controller {
    ...
    public function add() {
        ...
        if(in_array('sms',$actions)){
            $message="Dear {$enquiry['name']}, Thank You for your visit, Your visted is recorded.\nDescription: {$enquiry['description']}\n{$enquiry['note']}\nDated:{$enquiry['date']}\nRegard, Front Desk";
            $messages[]=array('text'=>$message,'to'=>$enquiry['contact']);
            $this->sendSMS($messages,true);
        }
        if(in_array('whatsapp',$actions)){
            $message="Dear {$enquiry['name']}, Thank You for your visit, Your visted is recorded.\nDescription: {$enquiry['description']}\n{$enquiry['note']}\nDated:{$enquiry['date']}\nRegard, Front Desk";
            $messages[]=array('text'=>$message,'to'=>$enquiry['contact']);
    ...
    function follow_up_insert() {
        ...
        $info=$this->enquiry_model->getenquiry_list($follow_up['enquiry_id']);
   

        $actions=$this->input->post('actions');
        if(in_array('sms',$actions)){
            $message="Dear {$info['name']}, Admission Follow-up ID: {$info['id']} is replied {$follow_up['response']}\nNote:{$follow_up['response']}\nDated:{$follow_up['date']}\nRegard, Front Desk";
            $messages[]=array('text'=>$message,'to'=>$info['contact']);
            $this->sendSMS($messages,true);
        }
        if(in_array('whatsapp',$actions)){
            $message="Dear {$info['name']}, Admission Follow-up ID: {$info['id']} is replied {$follow_up['response']}\nNote:{$follow_up['response']}\nDated:{$follow_up['date']}\nRegard, Front Desk";
