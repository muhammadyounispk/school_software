...
class Timetable extends Admin_Controller
{
    ...
    function CopyTimeTable()
    {
       ...
       $days=$this->input->post('copy');
       foreach ($days as $day=>$checked){
           if($checked) {
               $where="school_id='".getSchoolID()."' and class_id='$class_id'  and section_id='$section_id' and subject_group_id='$subject_group_id'";
               $del_sql="DELETE FROM subject_timetable  WHERE $where and day ='$day' ";
               db::query($del_sql);
               $query = "INSERT INTO `subject_timetable` (`school_id`, `day`, `class_id`, `section_id`, `subject_group_id`, `subject_group_subject_id`, `staff_id`, `time_from`, `time_to`, `start_time`, `end_time`, `room_no`, `session_id`, `created_at`)
                         SELECT `school_id`, '$day' as day , `class_id`, `section_id`, `subject_group_id`, `subject_group_subject_id`, `staff_id`, `time_from`, `time_to`, `start_time`, `end_time`, `room_no`, `session_id`, `created_at` from subject_timetable where  $where  and day='Monday' ";
        ...
        echo "Time Table is Copied Successfully";
