...
class Checkupdatesofsystem extends Admin_Controller 
{
    ...
    function isComLive()  {
        ...
        $wa=db::getRecords("SELECT * FROM `whatsapp_devices` where school_id='".getSchoolID()."' and is_active=1");
       if($wa){
        $wa_connected=0;
        foreach ($wa as $key => $whatsapp) {
        $id = $whatsapp['id'];
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => WHATSAPP_SERVER.'/scanQR?session='.$device.'&scan=false',
        ...
        $connected_wa=json_decode($response);
        if(isset($connected_wa->message)){
            $wa_connected=$wa_connected+1;
        }
        curl_close($curl);
       }
    ...
    $sms_connected="";
   $sms_devices=$this->sms_connected_devices;
        if($sms_devices){
            foreach ($sms_devices as $key => $sms_device) {
                $arr1=[$sms_device['id']."|".getSchoolInfo('default_sim')];
            $this->session->set_userdata("device_id",$arr1);
             
             $sim=$sms_device['sims'][getSchoolInfo('default_sim')];
              $sms_connected.=" $sim ";
            }
        }
        $arr=$this->session->userdata('device_id');
       echo json_encode(array("All_Devices"=>@$sms_device,'default_sim'=>@$sms_connected,'sms'=>empty($sms_devices) ? false : true , 'whatsapp'=>empty($wa_connected) ? false : true,'email'=>true ,'arr'=>$arr));
    }
