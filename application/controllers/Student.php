...
class Student extends Admin_Controller
{
    ...
    public function new_admission()
    {
        ...
        $students = $this->setting_model->ModuleRestriction();
        $enrolled = $this->setting_model->getTotalStudents();
        $this->total_allowd = $student_allowed = $students[1]['value'];
        ...
        $this->form_validation->set_rules('cnic', $this->lang->line('cnic_bform'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('father_cnic', $this->lang->line('father_cnic'), 'trim|required|xss_clean');
        ...
        if ($this->sch_setting_detail->guardian_phone) {
            $this->form_validation->set_rules('guardian_phone', " International Phone (e.g +923xxxxx)", 'trim|required|xss_clean|regex_match[/^[+]/]  ');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!$student_allowed) {
                $this->load->view('student/new_admission', $data);
            } else {
                if ($enrolled <= $student_allowed) {
                    $this->load->view('student/new_admission', $data);
                } else {
                    $this->load->view('student/student_enrollment_exceeded', $data);
                }
            ...
            if ($insert) {
                ...
                //print admission form 
                 if(in_array('download_admission_form',$actions)){
                    $this->session->set_flashdata('take_print',true);
                }
                ...
                redirect('student/view/'.$insert_id);
            } else {
    ...
    function autocomplete_search_student()
    {
        $term=$this->input->get('query');
        $student=db::getRecords("SELECT id as data,  concat(firstname,' ',lastname ,' Roll No:\n ', roll_no,'\n F/N: ',father_name ,' Phone: ', mobileno) as value from students where school_id='".getSchoolID()."' and (concat(firstname,' ',lastname ) LIKE '%$term%'  || roll_no LIKE '%$term%'  || mobileno LIKE '%$term%') order by firstname limit 20  ");

$data=array('query'=>$term,'suggestions'=>$student);

        echo json_encode($data);
    }

    function search_guardian()
    {
        $term=$this->input->get('query');
        $student=db::getRecords("SELECT guardian_cnic as data,  concat(guardian_name,' ',guardian_profession,' CNIC:\n ', guardian_cnic,'\n Mob: ',guardian_mobileno ) as value from guardians where school_id='".getSchoolID()."' and  (guardian_name LIKE '%$term%'  || guardian_cnic LIKE '%$term%'  || guardian_mobileno LIKE '%$term%') order by guardian_name limit 20  ");
...
$data=array('query'=>$term,'suggestions'=>$student);
    ...
    public function admissionform($id)
    {
        ...
        $student = $this->student_model->get($id);
        $url=$this->DownloadAdmissionForm($student);
        header("location:$url");
       
    }
    public function Sampleadmissionform()
    {

        if (!$this->rbac->hasPrivilege('student', 'can_view')) {
            access_denied();
        }
        $student = $this->student_model->getSample();
        
        $url=$this->DownloadAdmissionFormSample($student);
        header("location:$url");
    ...
    public function createPopUp()
    {
        ...
        if(  $id){
        $student_result = $this->student_model->getStudentbyID($id);
        $class_id=$student_result['class_id'];
         $data['section']=$this->section_model->getClassBySectionAll($class_id);
      
        }
        $data['student']=$student_result;
        $students = $this->setting_model->ModuleRestriction();
        $enrolled = $this->setting_model->getTotalStudents();
        $this->total_allowd = $student_allowed = $students[1]['value'];
        $this->session->set_userdata('top_menu', 'Student Information');
        ...
        $data['header_form']= $html = $this->LoadTemplate('school_header.html');
 
                $this->load->view('student/studentCreateForm', $data);
    ...
    {
        $pattern = '/^\+?\d{1,3}\s?\d{3,14}$/'; // regular expression pattern for phone numbers
    ...
    function deleteStudent(){
        ...
        db::query("DELETE from students where school_id=".getSchoolID()." and id=$id");
        echo "deleted";
    }
    ...
    public function studentCard()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            $this->load->view('layout/header', $data);
            $this->load->view('student/studentCard', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $class = $this->input->post('class_id');
            $section = $this->input->post('section_id');
    ...
    function Dob($date) : string
    {
        $tempDate = explode('/', $date);
        $day=strlen($tempDate[0])  < 2 ? "0".$tempDate[0] : $tempDate[0];
        $month=strlen($tempDate[1]) < 2 ? "0".$tempDate[1] : $tempDate[1];
        $year=strlen($tempDate[2]) < 2 ? "0".$tempDate[2] : $tempDate[2];
    ...
    public function importform()
    {
        ...
        if($fileType=='docx'){
            ...
            if(file_exists(ADMISSION_FORM)){
                unlink(ADMISSION_FORM);
            }

            if(move_uploaded_file($file['tmp_name'],ADMISSION_FORM)){
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">Admission Form has been Uploaded</div>');

            }
        }else{
            $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">Incorrect File Type</div>');
    ...
    public function UploadAdmissionForm()
    {
             $this->load->view('layout/header', '');
             $this->load->view('student/admission_form', '');
            $this->load->view('layout/footer', '');
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            foreach ($result[$i] as $key => $value) {
                                ...
                                if (isset($result[$i]['father_cnic'])) {
                                    if( strpos($result[$i]['father_cnic'], 'E') && strlen($result[$i]['father_cnic'])==13){
                                        $student_data[$i]['father_cnic'] = null;
                                        $error[$i]['father_cnic'] = array("code" => 404, " father_cnic in Scientific Notaion not accepted , please fix CSV, adjust bform in farmate 3230411223233412   not accepted In 32304E+10 ");
                                    }else{
                                      $fcnic=  $student_data[$i]['father_cnic'] = $result[$i]['father_cnic'];
                                        $father_id=db::getCell("SELECT id  from guardians where guardian_cnic='$fcnic'");
                                        if(!$father_id){
                                            if (isset($result[$i]['father_phone'])) {

                                            $data2['guardian_name']=$result[$i]['father_name'];
                                            $data2['guardian_cnic']=$result[$i]['father_cnic'];
                                            $data2['guardian_profession']="";
                                            $data2['school_id']=getSchoolID();
                                            $data2['guardian_email']=$result[$i]['email'];
                                            $data2['guardian_mobileno']="+9230".$result[$i]['father_phone'];
                                            $data2['caste']="";
                                            $data2['religion']="";
                                            $data2['address']="";
                                            $data2['balance']=0;
                                                ...
                                                $data['last_updated']=date('Y-m-d h:i:s a');
                                                 $query2=db::prepInsertQuery($data2,'guardians');
                                               $father_id=  db::insertRecord($query2);

                                            }
                                        }

                                        $student_data[$i]['parent_id'] = $father_id;
                                    
                                    }
                                } else {
                                    $student_data[$i]['father_cnic'] = null;
                                    $error[$i]['father_cnic'] = array("code" => 404, "father_cnic is missing column in csv ");
                                ...
                                if (isset($result[$i]['date_of_birth'])) {
                                    $student_data[$i]['dob'] = $this->Dob($result[$i]['date_of_birth']);
                                } else {
                                    $student_data[$i]['dob'] = null;
                                    $error = array("code" => 404, "date_of_birth is missing column in csv ");
                                    $error[$i]['date_of_birth'] = array("code" => 404, "date_of_birth is missing column in csv ");


                                }
                                
                                    $student_data[$i]['roll_no'] = $i.rand(1111,9999);
                    
                                if (isset($result[$i]['first_name'])) {
                                    $student_data[$i]['firstname'] = $result[$i]['first_name'];
                                } else {
                                    $student_data[$i]['first_name'] = null;
                                    $error = array("code" => 404, "first_name is missing column in csv ");
                                    $error[$i]['father_name'] = array("code" => 404, "father_name is missing column in csv ");

                                }


                                if (isset($result[$i]['last_name'])) {
                                    $student_data[$i]['lastname'] = $result[$i]['last_name'];
                                } else {
                                    $student_data[$i]['last_name'] = null;
                                    $error[$i]['last_name'] = array("code" => 404, "last_name is missing column in csv ");

                                }
                                if (isset($result[$i]['bform'])) {
                                   
                                        $student_data[$i]['cnic'] = $result[$i]['bform'];
                                    
                               
                                } else {
                                    $student_data[$i]['bform'] = null;
                                    $error[$i]['bform'] = array("code" => 404, "bform is missing column in csv ");

                                }
                               


                                if (isset($result[$i]['gender'])) {
                                    $student_data[$i]['gender'] = $result[$i]['gender'];
                                } else {
                                    $student_data[$i]['gender'] = "Male";
                                ...
                                if (isset($result[$i]['email'])) {
                                    ...
                                    $error[$i]['email'] = array("code" => 404, "email is missing column in csv ");


                                }
                                if (isset($result[$i]['mobileno'])) {
                                     if( strpos($result[$i]['mobileno'], 'E') && strlen($result[$i]['mobileno'])==13){
                                        $student_data[$i]['mobileno'] = null;
                                        $error[$i]['mobileno'] = array("code" => 404, " mobileno in Scientific Notaion not accepted , please fix CSV, adjust mobileno in farmate 3124545121   not accepted In 31244E+10 ");
                                    }
                                    else{
                                        $student_data[$i]['mobileno'] ="+923". $result[$i]['mobileno'];
                                    }
                                    


                                } else {
                                    $student_data[$i]['mobileno'] = "";
                                    $error[$i]['mobileno'] = array("code" => 404, "mobileno is missing column in csv ");


                                }


                                if (isset($result[$i]['father_name'])) {

                                    $student_data[$i]['guardian_name'] = $fname = $result[$i]['father_name'];
                                    $student_data[$i]['father_name'] = $fname;
                                    $student_data[$i]['guardian_is'] = "father";

                                } else {
                                    $student_data[$i]['father_name'] = "";
                                    $error[$i]['father_name'] = array("code" => 404, "father_name is missing column in csv ");

                                }

                                if (isset($result[$i]['father_phone'])) {
                                     $student_data[$i]['guardian_phone'] ="+923". $result[$i]['guardian_phone'];
                                     $student_data[$i]['father_phone'] ="+923". $result[$i]['father_phone'];
                                
                                } else {
                                    $student_data[$i]['father_phone'] = "";
                                    $error[$i]['father_phone'] = array("code" => 404, "father_phone is missing column in csv ");

                                }

                                if (isset($result[$i]['father_email'])) {
                                    $student_data[$i]['guardian_email'] = $result[$i]['father_email'];


                                } else {
                                    $student_data[$i]['father_email'] = "";
                                    $error[$i]['father_email'] = array("code" => 404, "father_email is missing column in csv ");

                                }

                                $student_data[$i]["mother_name"] = "N/A";
                                $student_data[$i]["mother_phone"] = "N/A";
                                $student_data[$i]["mother_occupation"] = "N/A";
                                $student_data[$i]["father_occupation"] = "N/A";
                                $student_data[$i]['admission_date'] = date("Y-m-d");
                                $student_data[$i]["admission_no"] = $class_id . "-" . $section_id . "-" . $student_data[$i]['roll_no'];
                                $student_data[$i]["rte"] = "no";
                                $student_data[$i]["cast"] = "N/A";
                                $student_data[$i]["religion"] = "Islam";
                                $student_data[$i]["height"] = "N/A";
                                $student_data[$i]["weight"] = "N/A";
                                $student_data[$i]["blood_group"] = "Islam";

                                $student_data[$i]["measurement_date"] = date("Y-m-d");


                                $student_data[$i]["note"] = "Enrolled from Excel Sheet";
                            ...
                            if ($this->form_validation->is_unique($adm_no, 'students.admission_no')) {
                                if (!$error) {
                                    ...
                                    $index = $index + $insert_id;
                                } else {
                                 
                                    $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">' . json_encode($error) . '</div>');
