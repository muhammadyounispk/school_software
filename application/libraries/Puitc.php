...
define("SERVER", "https://sms.isdigitalschools.com");
define("API_KEY", "48de3bcbf901edc67c3d4422832bbc45f81b9545");
...
public function __construct($array) {
    $CI =   &get_instance();
    $CI->db->select("*");
    $CI->db->where("school_id", getSchoolID());
    $CI->db->where("is_active", 1);
    $CI->db->from("whatsapp_devices");
    $rs = $CI->db->get();
    $this->wa_devices =$rs->result_array();
    ...
    $this->API_SIM = $array['sim'];
    $this->GATEWAY_DATA = $array['data'];
    $this->TO = $array['to'];
...
{
    $session= $this->wa_devices[0]['id']."_".$_SERVER['HTTP_HOST'];
    $delay= $this->wa_devices[0]['delay'];
    $wa=array();
   foreach ($messages as $key => $value) {
       $wa[$key]['to']=$value['number'];
       $wa[$key]['text']=$value['message'];
   }
   $data=json_encode($wa);
   $this->sendWhatsappText($session,$delay,$data);
...
function sendWhatsappText($session,$delay,$data="") {

    $curl = curl_init();
    
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://whatsapp.puitc.eu.org/sendMessageBulk',
